...
const __main__ = () => {
  ...
  app.on("message", async (message) => {
    ...
    if (data.startsWith(process.env.COMMAND_PLAY || "play")) {
      ...
      const query = data.substring(
        ((process.env.PREFIX || "!") + (process.env.COMMAND_PLAY || "play"))
          .length + 1
      );

      if (!isEmpty(query)) {
        vids = await scrapeYt.search(query, {
          type: "video",
          limit: parseInt(process.env.NUMBER_OF_YOUTUBE_VIDS || "4"),
        });
      ...
      try {
        song_choose = false;
        if (
          ("1" <= data && data <= process.env.NUMBER_OF_YOUTUBE_VIDS) ||
          "4"
        ) {
          await message.react(process.env.OK_SYMBOL || "👌");
          ...
          isPushed = true;
          await message.channel.send(
            `Pushing ${vids[parseInt(data) - 1].title} into the queue`
          );
        ...
        await message.channel.send(
          "This song is not available due to " + error
        );
